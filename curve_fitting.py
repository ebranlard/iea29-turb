...
from numpy import sqrt, pi, exp, cos, sin, log # for user convenience
    ...
    def set_model(self,func, **fun_kwargs):
        if callable(func):
            ...
            if func.find('predef:')==0:
                ...
                regex = r"\{(.*?)\}"
                matches = re.finditer(regex, formula, re.DOTALL)
                variables=[]
                for i, match in enumerate(matches):
                    for groupNum in range(0, len(match.groups())):
                        variables.append(match.group(1))
                        formula_eval = formula_eval.replace('{'+match.group(1)+'}','p[{:d}]'.format(i))
                        #formula = formula.replace('{'+match.group(1)+'}',match.group(1))
    ...
    def fit_data(self, x, y, p0=None, bounds=None):
        ...
        if self.model['coeffs'] is not None:
            if len(self.model['coeffs'])!=nParams:
                raise Exception('Inconsitent dimension between p0 (size {}) and the model parameters (size {})'.format(nParams,len(self.model['coeffs'])))

        # Bounds
        self.setup_bounds(bounds,nParams)

        # Initial conditions
        self.setup_guess(p0,bounds,nParams)
